...
async function loadCreativeAsset(storagePath: string) {
  ...
  if (!response.ok) {
    throw new Error("Creative asset download failed: " + response.status);
  }
...
while (!stop) {
  try {
    ...
    try {
      const output = await renderComposition(job.composition, loadCreativeAsset);
      ...
      await db("/creative_render_jobs?id=eq." + job.id + "&status=eq.rendering", {
        ...
        body: JSON.stringify({
          status: "failed",
          error:
            "Render failed. Check worker runtime, media assets, Chromium and FFmpeg; submit a new job to retry.",
          finished_at: new Date().toISOString(),
        }),
      });
